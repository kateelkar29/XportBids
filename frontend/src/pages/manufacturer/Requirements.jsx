...
import { FileText, Send, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
...
const ManufacturerRequirements = () => {
  ...
  const fetchRequirements = async () => {
    try {
      const response = await axios.get('/manufacturers/matched-requirements');
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 data-testid="requirements-title" className="text-4xl font-bold uppercase tracking-tight text-[#0F172A] mb-2\">Matched Tenders</h1>
        <p className="text-slate-600">Tenders matched to your products based on category, material, and capabilities</p>
      </div>

      {requirements.length === 0 ? (
        <div className="bg-white border border-[#E2E8F0] p-12 text-center">
          <FileText className="h-16 w-16 text-slate-300 mx-auto mb-4" />
          <p className="text-lg text-slate-600">No matched tenders at the moment.</p>
          <p className="text-sm text-slate-500 mt-2">Add more products or update your product specifications to see relevant tenders.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {requirements.map((item) => {
            const req = item.requirement;
            const match = item.match_indicator;
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-bold uppercase text-[#0F172A]">HSN: {req.hsn_code}</h3>
                      <Badge className="bg-[#F97316] text-white rounded-none">
                        {match.match_score}% Match
                      </Badge>
                    </div>
                    <p className="text-sm text-slate-600 mb-3">Quantity: {req.quantity} units | Category: {req.category}</p>
                    
                    {/* Match Indicators */}
                    <div className="flex flex-wrap gap-2 mb-4">
                      {match.category_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">Category Match</span>
                        </div>
                      )}
                      {match.material_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">Material Match</span>
                        </div>
                      )}
                      {match.certification_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">Certification Match</span>
                        </div>
                      )}
                      {match.moq_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">MOQ Compatible</span>
                        </div>
                      )}
                      {match.capacity_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">Capacity Match</span>
                        </div>
                      )}
                      {match.incoterm_match && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-green-50 border border-green-200 rounded-none">
                          <CheckCircle2 className="h-4 w-4 text-green-600" />
                          <span className="text-xs font-semibold text-green-700">Incoterm Match</span>
                        </div>
                      )}
                    </div>
                  </div>
